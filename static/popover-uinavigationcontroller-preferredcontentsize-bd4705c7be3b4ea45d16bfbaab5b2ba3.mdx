...
There are a few ways to work this out, but a simple one is to employ `UIContentContainer`. `UIContentContainer` is a protocol (adopted by `UIViewController`) which encapsulates all the methods which `UIViewController` uses to respond to changes in a child view controller's content.
