...
The `@NSManaged` property modifier is used for Core Data, but its effect is the same as Objc's `@dynamic` - it defers implementation of the property getters and setters. If we declare our layer property as `@NSManaged`, `CALayer` will be able to manage its getters and setters!
