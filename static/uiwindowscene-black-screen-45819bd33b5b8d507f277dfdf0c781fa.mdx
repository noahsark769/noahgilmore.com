...
However, multiple times now, I've thought I've set everything up correctly (scene delegate class, configuration with `delegateClass` set properly, correct Info.plist setup, etc), only to get a black screen on launch.
...
This has generally been the missing piece that resolves the black screen issue. [Here's a concrete example](https://github.com/noahsark769/NGSystemColorComparison/blob/master/NGSystemColorComparison/SceneDelegate.swift#L24).
