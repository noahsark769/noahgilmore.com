...
It seems that `UINavigationController` queues the animations for pushing on a new view controller and setting its own `preferredContentSize` based on the controller being pushed, so the push animation and the resize animation happen in series. Ideally, we'd like them to happen simultaneously.
