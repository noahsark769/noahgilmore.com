...
Each filter lists the description and parameters, pulled directly from the filter's `attributes` dict, which are generally more descriptive. For example, the `inputSharpness` parameter to [CICheckerboardGenerator](https://cifilter.io#CICheckerboardGenerator) says
