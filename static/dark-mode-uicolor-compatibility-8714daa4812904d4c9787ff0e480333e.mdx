...
`label` is only one example: there are 36 new color scheme agnostic `UIColor`s available in iOS 13+, organized into two groups:

1. [Element Colors](https://developer.apple.com/documentation/uikit/uicolor/ui_element_colors) (e.g. `label`)
2. [Standard Colors](https://developer.apple.com/documentation/uikit/uicolor/standard_colors) (`systemBlue`, `systemGray3`, etc).

(Skip to the bottom if you'd like to see a list of the new colors.)

## Compatibility
These new colors are all well and good, but most of us with existing apps will still be supporting devices with iOS 12 or lower, at least for a while. This means we'll probably be doing a lot of things like this:
...
This app collects all the `UIColor` objects - once we have those, we can use their red/green/blue/alpha components to [generate the implementation](https://github.com/noahsark769/NGSystemColorComparison/blob/master/NGSystemColorComparison/SystemColors.swift#L102) of `ColorCompatibility` that we want, using [Swift's #available syntax](https://docs.swift.org/swift-book/ReferenceManual/Attributes.html):
