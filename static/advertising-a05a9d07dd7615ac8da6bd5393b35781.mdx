...
2. Since there's no ad network, there's no ad targeting. There's no Facebook adware running on this site, and the site is [open source](https://github.com/noahsark769/noahgilmore.com). I use Google Analytics to track clicks on sponsored links so that I can make sure my analytics line up with those of the advertisers, but that's it.
3. If you use an ad blocker that supports blocking elements on the page by class name, you can opt out of ads entirely by blocking elements with the class name `sponsorship-opt-out-point`. For Adblock Plus, for example, the rule looks like `noahgilmore.com##.sponsorship-opt-out-point`.
