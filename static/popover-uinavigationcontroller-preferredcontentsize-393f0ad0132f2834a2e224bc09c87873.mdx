...
The issue we have here is that our green view controller is changing its `preferredContentSize`, but _the popover view controller doesn't know about that_. In an idea world, we'd have `PopoverPushController` respond to the green controller's content size change, and _update its own `preferredContentSize`_ in response.
