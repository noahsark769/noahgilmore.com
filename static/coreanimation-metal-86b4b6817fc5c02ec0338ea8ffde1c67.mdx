...
As an example: let's say we added an animation for our triangleScale, to take its value from 1 to 2. The first thing we have to do is make sure our `CALayer` subclass knows how to be instantiated as a presentation copy - CoreAnimation will call `init(layer:)` for this.
