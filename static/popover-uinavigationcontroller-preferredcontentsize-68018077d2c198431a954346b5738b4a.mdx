...
This post is going to describe a very specific but tricky interaction in UIKit, one which took me multiple days to work out how to implement. This one:
...
Specifically, it's two view controllers of different sizes, pushed on a `UINavigationController`, which is presented as a popover. The view controllers have different sizes (defined by autolayout), only one of them displays a nav bar, and the popover animates when their sizes change, including when they get pushed onto the nav stack.
...
Seems easy enough, but the kicker is when _the two view controllers are different sizes_. Surprisingly, this is tricky to implement in UIKit. In this post we'll discuss how to use a wrapper around `UINavigationController` to enable a UI like this:
...
<CaptionedImage max={300} filename="red-controller.png" alt="Red view controller" caption="View Controller A" />
<CaptionedImage max={300} filename="green-controller.png" alt="Green view controller" caption="View Controller B" />
