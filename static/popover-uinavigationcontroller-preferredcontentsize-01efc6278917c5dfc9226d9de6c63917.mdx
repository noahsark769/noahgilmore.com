...
## Updating the size
Everything's well and good now, so we can implement the resizing of the green controller on button tap. Our code isn't particularly complicated, but we'll use the same technique as the red controller for autolayout:
...
var isExpanded: Bool = false {
    didSet {
        self.heightConstraint.constant = self.isExpanded ? 600 : 400
...
But there's an issue when we try to update the green controller's height - the popover size doesn't animate!

<CaptionedImage max={500} filename="popover-animation-3.gif" alt="Green view controller updating height, but without animation." caption="Interesting again" />
