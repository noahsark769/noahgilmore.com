...
And we'll set the navigation bar hidden in `viewWillAppear`:
...
<CaptionedImage max={500} filename="red-controller-1.png" alt="Red view controller with the right size, pushing on a green view controller, whose size starts the same as the red controller but expands when finished pushing." caption="Also interesting" />
