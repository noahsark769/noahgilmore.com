...
In order to get a smooth animation, we'll need to employ `UIContentContainer`. `UIContentContainer` is a protocol adopted by `UIViewController`, and it encapsulates all the methods which `UIViewController` uses to respond to changes in a child view controller's content.
