...
The model layer never changes, but the presentation layer's `triangleScale` is automatically interpolated from 1 to 2 - but changes back to 1 after the animation ends, since we never actually set the model layer's value to 2. _Interpolation_ is the core value that we get for free when using CoreAnimation.

We still can't see the triangle growing, though! We'll need a bit more work to do that.
