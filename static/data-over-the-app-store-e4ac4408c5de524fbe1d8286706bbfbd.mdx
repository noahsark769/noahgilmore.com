...
In this post, I'm going to describe the technique behind how I implemented this without a backend or third party service, using `UIPasteboard`.
...
In javascript (depending on your browser) it's possible to programmatically copy to the clipboard using `document.execCommand("copy")` (see [MDN docs](https://developer.mozilla.org/en-US/docs/Web/API/Document/execCommand)). The API is a little finnicky - you have to construct a DOM element and fill it with the content you want to copy, and it only works on `input` and `textarea` elements, but it works! I tested this on iOS 11 and 12, but according to [caniuse](https://caniuse.com/#feat=clipboard) it looks like it's supported as far back as iOS 10.
...
Once we've got this working, it's easy enough to take some data, base64 encode it, and copy it to the clipboard in addition to the link's url parameter. Note that we include a timestamp of when the link was clicked - we'll need this for later. I'm using React here, but other frameworks or vanilla JS should be similar:
...
const TrestleBlogPostLink = (props) => {
    ...
    return (<DataLink data={data} href={"https://itunes.apple.com/us/app/trestle-the-new-sudoku/id1300230302?mt=8"}>{props.children}</DataLink>);
};
```

# Reading the link (app)
[UIPasteboard](https://developer.apple.com/documentation/uikit/uipasteboard) is a UIKit class which allows your app to integrate with iOS's copy and paste functionality. If you've ever seen a "copy link" button in an app, it's probably using `UIPasteboard`.

You can access the current contents of the general pasteboard (the one that gets updated when the user copies and pastes) using `UIPasteboard.general` - `UIPasteboard.general.strings` gives you the current text contents (pasteboard can also hold images, colors, etc).
