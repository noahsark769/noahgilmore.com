...
The description of `CIHueSaturationValueGradient` is "Generates a color wheel that shows hues and saturations for a specified value".

For a given value of lightness (dark/light), this filter's output image will be a wheel of all the hues and saturations for that lightness in a given color space. Code that looks like this:
