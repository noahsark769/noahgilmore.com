...
And in our tests, we've defined a util to test it (less complicated than `assertRoundTrip`, but the same idea):
...
<CaptionedImage filename="xcodeshared-2.png" max={300} caption="" alt="Xcode UI with 'New Unit Test' menu item selected" />
...
<CaptionedImage filename="xcodeshared-5.png" max={300} caption="" alt="Xcode UI with the 'Run example app tests' menu item selected" />
...
This one is a little harder: navigating to the report navigator and looking at the build log, we see the real error: `Failed to load module 'XCTest'`.
...
We haven't specified that the `ExampleApp` target is a dependency of `SharedTestUtils`, 
