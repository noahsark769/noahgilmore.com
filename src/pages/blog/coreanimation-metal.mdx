...
> Note: layers which back `UIView`s don't have these implicit animations by default since `UIView`s don't provide an action through their layer delgate method. More on this later.
