...
But `label` is only one example: there are 36 new color scheme agnostic `UIColor`s available in iOS 13+, organized into [Element Colors](https://developer.apple.com/documentation/uikit/uicolor/ui_element_colors) (e.g. `label`) and [Standard Colors](https://developer.apple.com/documentation/uikit/uicolor/standard_colors) (`systemBlue`, `systemGray3`, etc).
